...
$can3 = $row['tongji'];
if (!empty($can2)){
	$sql1 .= " and user_id not in ($can3)";
}

$sql = "select * from user_info where $sql1 order by diamond DESC LIMIT 10";
//echo $sql;
...
while($row = mysql_fetch_array($res)){
	...
	$show[$key]['total'] = (int)$row['diamond'];
	$show[$key]['type'] = 1;
...
//print_r($show); 
/*
$sql = "select *,(deposit+gold) as caifu from user_info where $sql1 order by deposit DESC LIMIT 20";
//echo $sql;
$res = mysql_query($sql, $conn2);
while($row = mysql_fetch_array($res)){
	//echo $key."<br>";
