...
class OtherAction extends BaseAction {
	...
	public function jinbi(){
		...
		if (!empty($user_id) && !(empty($gold))){
			...
			if (empty($gold)){
				$this->error('房卡不能字段不能为空');
				exit;
			}
			
			if ($gold>2000000000 or $gold<-2000000000){
				$this->error('房卡不能大于20亿小于-20亿');
				exit;
			}
			//判断当前数量是否足够
			$user_name = $_SESSION['username'];
			$res = $js_row->where("username='$user_name'")->find();
			//超级管理员
			if( $res && is_array($res)  && $res['user_js'] == 9 )
			{
				//查找输入的用户id是否存在
				$userinfo_table = "mj_userinfo";
				$userinfo_table_row = M($userinfo_table,'',DB_CONFIG2);
				$res_userinfo = $userinfo_table_row->where("user_id='$user_id'")->find();
				//if(!mysql_num_rows($res_userinfo))
				...
				$para['cardnum'] =(int) $gold;
				$para['type'] = 1;
				$url = "http://10.135.72.229:6021/scmjsvr";
				...
				$data['addtime']   = time();
				$data['notify_date']   = date("Y-m-d H:i:s", time());
				$data['status'] =  1;
			...
			//代理商
			else if ($res && is_array($res)  && $res['user_js'] != 9 && $res['card_num'] >= $gold)
			{
				//查找输入的用户id是否存在
				$userinfo_table = "mj_userinfo";
				$userinfo_table_row = M($userinfo_table,'',DB_CONFIG2);
				...
				$data['notify_date']   = date("Y-m-d H:i:s", time());
				$data['status'] =  1;
				$result11 =  $js_row->where("username='$user_name'")->setInc('card_num',-$gold);
				if($result11)
				{
					$row->add($data);
					$lib_display = $this->By_tpl."/jinbi";
			...
			$user_name = $_SESSION['username'];
			$count = $row->where("czz='$user_name'")->count('id');
			$Page       = new Page($count,20);//实例化分页类传入总记录数和每页显示的记录数		
			$show       = $Page->show();// 分页显示输出
			$list = $row->where("czz='$user_name'")->order('id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
			foreach($list as $key=>$value){
				$list[$key]['notify_status']="1";
			...
			$this->assign('user_js',$_SESSION['user_js']);
	...
	public function userbase(){
		$table = "fx_sell_card"; //记录
		$row = M($table);
		
		$table_js = "users"; //管理员表
		$table_row = M($table_js);
		//获取输入框数据
		$user_id = I("user_id");
		$gold = I("gold");
		$meno = I("meno");
		...
		//购买者
		$res = $table_row->where("username='$user_id'")->find();
		...
		if (!empty($user_id) && !(empty($gold))){
			...
			if ($gold>2000000000 or $gold<-2000000000){
				$this->error('不能大于20亿小于-20亿');
				exit;
			}
			$status = 1;
			$data = array();
			$data['user_js']   = $user_id ;
			//判断当前数量是否足够
			$user_name = $_SESSION['username'];
			$resSalesman = $table_row->where("username='$user_name'")->find();
			//超级管理员:9, 总代理：10
			if( $resSalesman && is_array($resSalesman)  && $resSalesman['user_js'] == 9 )
			{
				$data['gold']   = $gold;
				$data['meno'] =  $meno;
				$data['czz'] =  $_SESSION['username'];
				...
				$result = $row->add($data);
				if($result){
					$result2 = $table_row->where("username='$user_id'")->setInc('card_num',$gold);
					if($result2){
			...
			//代理商
			else if ($resSalesman && is_array($resSalesman)  && $resSalesman['user_js'] != 9 && $resSalesman['card_num'] >= $gold)
			{
				$data['gold']   = $gold;
				$data['meno'] =  $meno;
				$data['czz'] =  $_SESSION['username'];
				...
				//
				$user_name = $_SESSION['username'];
				$result11 =  $table_row->where("username='$user_name'")->setInc('card_num',-$gold);
				if($result11)
				{
				}
				else
	...
	public function jinbi_ajax(){
		$table = "fx_other_jinbi";
		$row = M($table);
		$id = I("id");
		if (intval($id) > 0){
			$url = DB_HOST."/Pay/jinbi.php?id=".$id;
			//echo $url; 
			$jinbi_result = curlGET($url);
			...
			if ($notify_status == "1"){
				...
				$result1 = $row->where("id=".$id)->save($jinbi);
				$result2 = $row->where("id=".$id)->setInc('notify_times',1);
			}
			return true;
		}else{
			return false;
	...
	public function userlist(){
		...
		if ($act=="info"){
			...
			$table3 = "log_game_record_log_".date("Ym", $time11);
			$row3 = M($table3, '', DB_CONFIG2);
	...
	public function touxiang(){
		...
		if (!empty($_POST)){
			...
			foreach($_POST['uid'] as $key => $val){
				...
				if (!empty($val) && $_POST[$show]=="1"){
					...
					//删除头像
					$key1 = "Swx!%03hc2~4168209k";
					$user = $row->where("user_id=".$val)->find();
					$head_picture = str_replace("tx/", "", $user['head_picture']);
					$head_picture .= ".jpg";
					//echo $head_picture.$key1."<br>";
					$sign = md5($head_picture.$key1);
					//echo $sign."<br>";
					$url = "http://api.pic.kk520.com:9101/work/delpic.php?pic=".$head_picture."&sign=".$sign;
					//echo $url."<br>"; 
					$result = curlGET($url);
					//echo $result."<br>"; 
		...
		foreach($user as $key => $val){
			$user[$key]['picflag'] = ($val['head_picture'] == "tx/weigui") ? 1 : 0;
		}
		//dump($row->_sql());
		...
		if (!empty($beginTime)){
			...
			if ($beginTime < $today){
				...
				foreach($res1 as $key1 => $val1){
					...
					/*
					if ($key1 >= 25){
						$url = "index.php?m=Other&a=channel&beginTime=".$beginTime;
						header("location:$url");
						exit;
					}*/
				}
	...
	//获取渠道数据
	public function channel_ios(){

		$row = M("fx_online_tongji3");
		$row4 = M("payment");
		$beginTime = I("beginTime");
		$today = date("Y-m-d");
		...
		if (!empty($beginTime)){
			...
			if ($beginTime < $today){
				...
				$table2 = "log_login_".date("Ymd", $time1);
				$row2 = M($table2, '', DB_CONFIG3);
				$table22 = "log_game_".date("Ymd", $time1);
				$row22 = M($table22, '', DB_CONFIG3);
				$model3 = M('', '', DB_CONFIG3);
				//$res10 = $row1->field('user_id')->limit(0,100)->select();
				...
				//判断是否分析完
				$row11 = M("ios_union.union_finish", '', DB_CONFIG3);
				$count11 = $row11->where("date='$date1' and flag=1")->count();
				if ($count11 > 0){
					//开始
					$row12 = M("ios_union.union_user_channel", '', DB_CONFIG3);
					
					$res1 = $row12->where("date='$date1' and flag=0")->select(); 
					foreach($res1 as $key1 => $val1){
						if (!empty($val1['channel'])){
							...
							if ($total == 0){
								//新增用户
								$count1 = $row1->where("$sql1 and register_date>='$date1' and register_date<'$date2' and user_id in (".$val1['user_id'].")")->count('user_id');
								//活跃用户
								$temp1 = explode(",", $val1['user_id']);
								$count2 = count($temp1);
								//新增有效用户
								$sql11 = "SELECT COUNT(nums) AS total FROM (SELECT COUNT(user_id) AS nums FROM $table22 WHERE user_id in (".$val1['user_id'].") GROUP BY user_id ) AS t1 WHERE nums>=3";
								$info = $model3->query($sql11);
								$count3 = $info[0]['total'];
								//dump($row1->_sql());
								//echo "<br>".$count3."<br>";
								//独立付费用户
								$count4 = $row3->where("order_create_time>='$time1' and order_create_time<'$time2' and payment_status in ('1','-2') and user_id in (".$val1['user_id'].")")->count('distinct user_id');
								//新增付费用户
								$res9 = $row1->field('user_id')->where("$sql1 and register_date>='$date1' and register_date<'$date2' and user_id in (".$val1['user_id'].")")->select();
								$sql40 = "";
								foreach($res9 as $key9 => $val9){
									$sql40 .= (empty($sql40)) ? $val9['user_id'] : ",".$val9['user_id'];
								}
								...
								//总收入
								$count13 = $row3->where("order_create_time>='$time1' and order_create_time<'$time2' and payment_status in ('1','-2') and user_id in (".$val1['user_id'].")")->sum('result_money');
								if (empty($count13)) $count13 = 0;  else $count13 = $count13 / 100;
								//付费率(付费用户/活跃用户)
								$count6 = ($count2==0) ? 0 : round($count4/$count2,3) * 100;
								//付费用户ARPU(总收入/付费用户)
								...
								$time3 = $time1 - 60 * 60 * 24;
								$date3 = date("Y-m-d", $time3);
								$res10 = $row1->field('user_id')->where("$sql1 and register_date>='$date3' and register_date<'$date1' and user_id in (".$val1['user_id'].")")->select();
								$sql4 = "";
								$sum1 = 0;
								foreach($res10 as $key10 => $val10){
									$sql4 .= (empty($sql4)) ? $val10['user_id'] : ",".$val10['user_id'];
								...
								if ($sum1 == 0){
									...
									$sum2 = $row2->where("$sql1 and login_date>='$date1' and login_date<'$date2' $sql4 and user_id in (".$val1['user_id'].")")->count('distinct user_id');
									$count10 = ($sum1==0) ? 0 : round($sum2/$sum1, 3)*100;
								}
								//7日留存
								$time3 = $time1 - 60 * 60 * 24 * 7;
								...
								$time4 = $time3 + 60 * 60 * 24;
								$date4 = date("Y-m-d", $time4);
								$res10 = $row1->field('user_id')->where("$sql1 and register_date>='$date3' and register_date<'$date4' and user_id in (".$val1['user_id'].")")->select();
								//dump($row1->_sql());
								$sql4 = "";
								$sum1 = 0;
								foreach($res10 as $key10 => $val10){
								...
								foreach($payment as $key2 => $val2){
									if (empty($sql40)) {$count = 0;} else{
										$count = $row3->where("order_create_time>='$time1' and order_create_time<'$time2' and payment_status in ('1','-2') $sql40 and user_id in (".$val1['user_id'].") and payment_id=".$val2['payment_id']." and package_id=".$val1['channel'])->sum('result_money');
										if (empty($count)) $count = 0;  else $count = $count / 100;
									}
									$payment[$key2]['count'] = $count;
								}
						...
						$data = array("flag" => 1);
						$row12->where("id=".$val1['id'])->save($data); 
					}
				...
				//exit;
				$this->success('添加成功', U('Other/channel_ios'));
				exit;
			}else{
				$this->error('小于当天的数据不保存');
				exit;
			...
			$this->assign('beginTime',$beginTime);
			$this->assign('left_css',"41");
			$this->display("Other/channel_ios");
		}
	}

	//异步发送渠道数据
	public function channel_ios_ajax(){
		$row = M("fx_online_tongji3");
		$list = $row->where('flag=0')->order('addtime desc')->select();
		$gameid = "102";
		$url = CHANNEL_RUL;
