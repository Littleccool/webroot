			...
			for ($i=1; $i<=$day_jian; $i++){
				...
				if ($total == 0){
					...
					foreach($res2 as $key2 => $val2){
						...
						$sumall += $count1;
						$sum1 += $count2;
                        $sql11 .= (empty($sql0)) ? $val2['payment_id'] : ",".$val2['payment_id'];
					}
                    //其它新增支付
                    $sql11 = " and payment_id not in ($sql11)";
                    $count11 = $row1->where("order_create_time>='$time1' and order_create_time<'$time2' and payment_status in ('1','-2') and payment_id='".$val2['payment_id']."' $sql1 $sql11")->sum('result_money');
                    if (empty($count11)) $count11 = 0;
                    $count12 = $row1->where("order_create_time>='$time1' and order_create_time<'$time2' and payment_status in ('1','-2') and payment_id='".$val2['payment_id']."' $sql1 $sql11")->count('id');
