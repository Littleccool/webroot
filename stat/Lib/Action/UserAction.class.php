...
class UserAction extends BaseAction {
	...
	public function user_add(){
		if(!empty($_POST)){
			...
			$user_js = $_POST['user_js'];
			if(!$this->check_js($user_js)){
				$this->error('新用户权限不能比自己高');
				exit;
			}
			$data = array();
	...
	public function user_edit(){
		...
		if(!empty($_POST)){
			...
			//权限校验
			if(!$this->check_js($_POST['user_js'])){
				$this->error("权限不够");
				exit;
			}
	...
	public function jiaose_add(){
		if(!empty($_POST)){
			...
			$post_str['jinbi'] = $_POST['jinbi'];
			$post_str['diamond'] = $_POST['diamond'];
			$post_str['deposit'] = $_POST['deposit'];
	...
	public function userreward(){
		...
		$tablename = M('pay_reward');
		
		$user_id = I('user_id');
		$sql1 = "";
		if (!empty($user_id)) $sql1 .= " and user_id=".trim($user_id);
		
		import('ORG.Util.Page');
		$count = $tablename->where("1".$sql1)->count('id');
		$Page       = new Page($count,15);//实例化分页类传入总记录数和每页显示的记录数		
		$show       = $Page->show();// 分页显示输出
		
		$list = $tablename->where("1".$sql1)->order('id')->limit($Page->firstRow.','.$Page->listRows)->select();
		
		foreach($list as $key=>$value){
			
			if($value['cate']==1){
				$list[$key]['showcate']='金币';
			}elseif($value['cate']==2){
				$list[$key]['showcate']='钻石';
			}else{
				$list[$key]['showcate']='';
			}
			
			$list[$key]['status']=($value['status']=="1") ? "正常" : "停用";
		}
		...
		$this->display('User:userreward_list');
	}
	
	//添加用户充值奖励
	public function userreward_add(){
		if(!empty($_POST)){
			...
			$tablename = M('pay_reward');
			//判断是否已添加
			$count = $tablename->where("user_id=".$_POST['user_id'])->count();
			if ($count > 0){
				$this->error('该用户已添加');
				exit;
			}
			...
			$data['bili'] =  trim($_POST['bili']);
			$data['status'] =  trim($_POST['status']);
			$data['addtime']   = date("Y-m-d H:i:s");
			
			$result = $tablename->add($data);
			//dump($employee->_sql()); exit;
			if($result){
				//增加操作记录
				$logs = C('MANAGE_MSG_ADD_SUCCESS');
				...
				$this->success('添加成功',U('User/userreward'));
			}else{
				//增加操作记录
				$logs = C('MANAGE_MSG_ADD_FALSE');
				adminlog($logs);
			...
			$this->assign('left_css',"1");
			$this->display('User:userreward_add');
		}
		
	}

	//用户充值奖励更新
	public function userreward_edit(){
		...
		if(!empty($_POST)){
			...
			$data['addtime']   = date("Y-m-d H:i:s");
			$where['id'] = intval($id);
			$result = $tablename->where($where)->save($data);
			if($result){
				//增加操作记录
				$logs = C('MANAGE_MSG_EDIT_SUCCESS');
				$remark = "(修改用户:".$userid.",".json_encode($data).")";
				...
				$this->success('更新成功',U('User/userreward'));
			}else{
				//增加操作记录
				$logs = C('MANAGE_MSG_EDIT_FALSE');
				adminlog($logs);
			...
			$info = $tablename->where("id=".$id)->find();
			$this->assign('info', $info);
			...
			$this->assign('left_css',"1");
			$this->display('User:userreward_edit');
		}
	}
	...
	public function userreward_delete(){
		if(empty($_GET)){ 
			...
			$tablename = M('pay_reward');
			$where['id'] = $id;
			$result = $tablename->where($where)->delete();
			if($result){
				//增加操作记录
				$logs = C('MANAGE_MSG_DEL_SUCCESS');
				$remark = "(删除用户:".$userid.")";
