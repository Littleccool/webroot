                        ...
                        <form class="form-horizontal" method="POST" action="" id="editUserForm">
                           <input type="hidden" name="id" value="<?php echo ($info["id"]); ?>">
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="js_name" name="js_name" placeholder="角色名称" value="<?php echo ($info["js_name"]); ?>">
                                </div>
                            </div>
							<div class="form-group">
                                <label for="username" class="col-sm-2 control-label">可管理金币</label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="jinbi" name="jinbi" placeholder="可管理金币" value="<?php echo ($info["jinbi"]); ?>">
                                </div>
                            </div>
							<div class="form-group">
                                <label for="username" class="col-sm-2 control-label">可管理钻石</label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="diamond" name="diamond" placeholder="可管理钻石" value="<?php echo ($info["diamond"]); ?>">
                                </div>
                            </div>
							<div class="form-group">
                                <label for="username" class="col-sm-2 control-label">可管理存款</label>
                                <div class="col-sm-4">
                                    <input type="text" class="form-control" id="deposit" name="deposit" placeholder="可管理存款" value="<?php echo ($info["deposit"]); ?>">
                                </div>
                            </div>
							<div class="form-group">
                                <label for="username" class="col-sm-2 control-label">角色类型</label>
                                <div class="col-sm-4">
                                    <input type="radio" name="js_flag" value="0" <?php if($info["js_flag"] == 0): ?>checked="checked"<?php endif; ?>>普通管理员 
									&nbsp;&nbsp;<input type="radio" name="js_flag" value="1" <?php if($info["js_flag"] == 1): ?>checked="checked"<?php endif; ?>>超级管理员(拥有所有权限)
                                </div>
                            </div>
							
							<?php if(is_array($caozuo)): $k = 0; $__LIST__ = $caozuo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($k % 2 );++$k;?><div class="form-group">
                                <label for="username" class="col-sm-2 control-label"><?php if($k == 1): ?>权限管理<?php endif; ?></label>
                                <div class="col-sm-4">
                                    <input type="checkbox" name="<?php echo ($vo1["id"]); ?>" value="1" <?php if($vo1["checked"] == 1): ?>checked="checked"<?php endif; ?>><?php echo ($vo1["lanmu_name"]); ?>
									&nbsp;&nbsp;
                                </div>
                            </div>
								<?php if(is_array($vo1['sub'])): $i = 0; $__LIST__ = $vo1['sub'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo11): $mod = ($i % 2 );++$i;?><div class="form-group">
									...
									<div class="col-sm-4">
										&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="<?php echo ($vo11["id"]); ?>" id="<?php echo ($vo11["id"]); ?>" onclick="subsel('<?php echo ($vo11["id"]); ?>')"  value="1" <?php if($vo11["checked"] == 1): ?>checked="checked"<?php endif; ?>>--<?php echo ($vo11["lanmu_name"]); ?>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
													
										<?php if(is_array($vo11['caozuo'])): $i = 0; $__LIST__ = $vo11['caozuo'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i % 2 );++$i;?>&nbsp;&nbsp;<input type="checkbox" name="<?php echo ($vo11["id"]); ?>_power[]" value="<?php echo ($vo2["power_do"]); ?>" <?php if($vo2["checked"] == 1): ?>checked="checked"<?php endif; ?> ><?php echo ($vo2["power_name"]); endforeach; endif; else: echo "" ;endif; ?>
									</div>
